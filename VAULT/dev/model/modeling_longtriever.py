...
class Longtriever(BertModel):
    ...
    def forward(
        ...
        input_shape = input_blocks.size()
        batch_size, sent_num, seq_length = input_shape

        # Weird shapes with input blocks... figure out if I want 66 or 22???
        input_ids = input_ids.view(batch_size*sent_num,512)
        attention_mask = attention_mask.view(batch_size*sent_num,512)
        input_blocks = input_blocks.view(batch_size*sent_num,seq_length).unsqueeze(1)
        ...
        special_embedding_output = self.embeddings(input_ids=input_ids)
        cls_token = special_embedding_output[:, 0:1, :]  
        sep_token = special_embedding_output[:, -1:, :] 

        station_placeholder = torch.zeros((special_embedding_output.shape[0], 1, special_embedding_output.shape[-1]),dtype=special_embedding_output.dtype,device=special_embedding_output.device)
        embedding_output = torch.cat([station_placeholder, cls_token, input_blocks, sep_token], dim=1)
