...
exp_name=$task'_med-hierarchical'
model_path=$STORAGE_DIR'/models/vault/'$exp_name
echo $model_path
if [[ ! -d $model_path ]]; then
  mkdir -p $model_path
...
torchrun --nproc_per_node=4 train.py \
        ...
        --max_corpus_sent_num 8 \
        --model_type hierarchical \
        --model_name_or_path $STORAGE_DIR/models/vault/pretrained/BiomedNLP-BiomedBERT-base-uncased-abstract-fulltext \
        --output_dir $model_path \
        --do_train True \
        --num_train_epochs 10 \
        --save_strategy epoch \
...
torchrun --nproc_per_node=4 train.py \
        ...
        --max_corpus_sent_num 8 \
        --model_type hierarchical \
        --model_name_or_path $STORAGE_DIR/models/vault/wikipedia_med-hierarchical \
        --output_dir $model_path \
        --do_train True \
        --num_train_epochs 10 \
        --save_strategy epoch \
